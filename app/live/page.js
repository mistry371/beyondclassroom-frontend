    ...
    <div className="min-h-screen bg-gradient-to-br from-dark via-dark-100 to-dark">
      ...
      <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 text-center">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-navy mb-3">Live Classes — Enrolled Students Only</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-3xl font-bold text-navy">Live Classes</h1>
              <p className="text-muted">Interactive live sessions with expert instructors</p>
            </div>
          </div>
        </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {liveClasses.map(cls => (
                <div key={cls._id} className="bg-dark/50 rounded-xl p-4 flex items-center justify-between">
                  <div>
                    <h3 className="text-navy font-bold">{cls.title}</h3>
                    <p className="text-muted text-sm">{cls.instructor} • {cls.enrolled || 0}/{cls.maxStudents} students</p>
                  </div>
                  <a href={cls.zoomLink} target="_blank" rel="noreferrer"
                    className="px-4 py-2 bg-red-500 text-navy rounded-lg font-medium hover:bg-red-600 transition-all flex items-center gap-2"
                  >
        ...
        <div className="flex gap-3 mb-6">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`px-5 py-2 rounded-lg font-medium transition-all ${activeTab === tab.id ? 'bg-gradient-to-r from-primary to-secondary text-white' : 'bg-white text-muted hover:bg-dark-200'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        {activeTab === 'upcoming' && (
          upcomingClasses.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-muted text-xl">No upcoming classes scheduled</p>
              <p className="text-muted text-sm mt-2">Check back soon for new sessions</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {upcomingClasses.map((cls, index) => (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-navy mb-1">{cls.title}</h3>
                  <p className="text-primary text-sm mb-3">{cls.topic}</p>
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center gap-2 text-muted text-sm"><Users className="h-4 w-4" />{cls.instructor}</div>
                    <div className="flex items-center gap-2 text-muted text-sm"><Calendar className="h-4 w-4" />{cls.date} at {cls.time}</div>
                    <div className="flex items-center gap-2 text-muted text-sm"><Clock className="h-4 w-4" />{cls.duration}</div>
                  </div>
                  {cls.zoomLink ? (
                    <a href={cls.zoomLink} target="_blank" rel="noreferrer"
                    ...
                    <div className="w-full px-4 py-2 bg-academic text-muted rounded-lg text-center text-sm">
        ...
        {activeTab === 'recordings' && (
          recordings.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-muted text-xl">No recordings available yet</p>
              <p className="text-muted text-sm mt-2">Recordings of past live sessions will appear here</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {recordings.map((cls, index) => (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className="px-3 py-1 bg-gray-500/20 text-muted text-xs rounded-full font-medium">Recorded</span>
                  </div>
                  <h3 className="text-xl font-bold text-navy mb-1">{cls.title}</h3>
                  <p className="text-secondary text-sm mb-3">{cls.topic}</p>
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center gap-2 text-muted text-sm"><Users className="h-4 w-4" />{cls.instructor}</div>
                    <div className="flex items-center gap-2 text-muted text-sm"><Calendar className="h-4 w-4" />{cls.date}</div>
                    <div className="flex items-center gap-2 text-muted text-sm"><Clock className="h-4 w-4" />{cls.duration}</div>
                  </div>
                  {cls.recordingUrl ? (
