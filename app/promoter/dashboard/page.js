      ...
      <main className="max-w-7xl mx-auto px-4 py-8 pb-28 md:pb-8">
        {notification && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
            className="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl text-green-800 text-sm"
          >
            {notification}
          </motion.div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-3 mb-8">
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white border border-slate-200">
            ...
            <span className="font-semibold text-slate-700">{promoter.streak || 0} day streak</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-gradient">
            <Award className="h-5 w-5 text-white" />
            <span className="font-semibold font-mono text-white">{promoter.referralCode}</span>
          </div>
          {badges.map((b) => (
            <span key={b.id} className="px-3 py-1.5 rounded-full bg-white border border-slate-200 text-slate-700 text-sm" title={b.label}>
              {b.icon} {b.label}
            </span>
          ))}
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              className="rounded-2xl bg-white border border-slate-200 shadow-sm p-5 hover:border-primary/30 transition-colors"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-2 rounded-2xl bg-white border border-slate-200 p-6">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2 text-slate-700">
              <TrendingUp className="h-5 w-5 text-primary" /> Referral growth (7 days)
            </h3>
            <div className="flex items-end gap-2 h-36">
              {chartBars.map((h, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <motion.div initial={{ height: 0 }} animate={{ height: `${Math.max(h, 6)}%` }} transition={{ delay: i * 0.06 }}
                    className="w-full bg-brand-gradient rounded-t-md min-h-[8px]"
                    style={{ maxHeight: '100%' }}
                  />
          ...
          <div className="rounded-2xl bg-white border border-slate-200 p-6">
            ...
            <ul className="space-y-2">
              {leaderboard.length === 0 ? (
                ...
                <li key={row.rank} className="flex justify-between text-sm py-2 border-b border-slate-100">
                  <span className="text-slate-700">#{row.rank} {row.name}</span>
                  <span className="text-primary font-semibold">{row.earnings}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-8">
          <div className="rounded-2xl bg-white border border-slate-200 p-6">
            <h3 className="font-bold text-lg mb-4 flex items-center gap-2 text-slate-700">
              <Link2 className="h-5 w-5 text-primary" /> Referral toolkit
            </h3>
            <div className="flex gap-2 mb-4">
              <input readOnly value={promoter.referralLink || ''}
                className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-800"
            ...
            {qrUrl && (
              <div className="flex flex-col sm:flex-row gap-6 items-center mb-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                <img src={qrUrl} alt="Referral QR code" width={140} height={140} className="rounded-lg bg-white p-2 interactive" />
                <p className="text-slate-500 text-sm">Scan to register with your referral code automatically applied.</p>
              </div>
            )}
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={shareWhatsApp} className="flex items-center gap-2 px-4 py-2.5 bg-[#25D366]/10 text-[#25D366] rounded-xl text-sm font-semibold hover:bg-[#25D366]/20">
                <MessageCircle className="h-4 w-4" /> WhatsApp
              </button>
              <button type="button" onClick={copyLink} className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm hover:bg-slate-200">
                <Share2 className="h-4 w-4" /> Share link
              </button>
              <button type="button" onClick={() => setShowTour(true)} className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm hover:bg-slate-200">
          ...
          <div className="space-y-6">
                  ...
                  return (
                    <div key={m.label}>
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="text-primary">{m.current}/{m.target}</span>
                      </div>
            ...
            <div className="rounded-2xl bg-white border border-slate-200 p-6">
              ...
              />
              <button type="button" onClick={handleWithdraw} className="w-full py-3 bg-accent-gradient text-white rounded-xl font-bold">
                Request Payout
              </button>
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="rounded-2xl bg-white border border-slate-200 p-6">
            <h3 className="font-bold mb-4 text-slate-700">Payout history</h3>
            {payouts.length === 0 ? (
              ...
              <ul className="space-y-2 max-h-48 overflow-y-auto scroll-smooth-touch">
                {payouts.slice(0, 10).map((p, i) => (
                  <li key={i} className="flex justify-between text-sm border-b border-slate-100 pb-2">
                    <span className="text-slate-700">₹{p.amount} <span className="text-slate-400">({p.status})</span></span>
          ...
          <div className="rounded-2xl bg-white border border-slate-200 p-6">
            <h3 className="font-bold mb-4 text-slate-700">Recent activity</h3>
            {history.length === 0 ? (
              ...
              <ul className="space-y-2 max-h-48 overflow-y-auto scroll-smooth-touch">
                {history.map((h, i) => (
                  <li key={i} className="flex justify-between text-sm border-b border-slate-100 pb-2">
                    <span className="capitalize text-slate-700">{h.type} {h.userName ? `— ${h.userName}` : ''}</span>
                    <span className="text-primary font-semibold">₹{h.amount}</span>
                  </li>
                ))}
              </ul>
            )}
